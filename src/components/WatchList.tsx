...
export function WatchList() {
  ...
  const toggleSort = (key: 'price' | 'changePct' | 'amplitude') => {
    if (sortKey === key) {
      if (!sortAsc) setSortAsc(true);     // desc → asc
      else { setSortKey('default'); setSortAsc(false); } // asc → 默认
    } else {
      setSortKey(key); setSortAsc(false); // 新字段，从降序开始
    }
  };
  ...
  const filteredList = useMemo(() => {
    ...
    let list = !q
      ? [...watchlist]
      : watchlist.filter((w) => {
          const name  = (quotes?.find((d) => d.code === w.code)?.name ?? w.alias ?? '').toLowerCase();
    ...
    return list;
  }, [sortKey, sortAsc, searchQuery, watchlist, quotes, getAmplitude]);
    ...
    <div className="flex-1 max-h-[1025px] flex flex-col rounded-lg border border-ink-200 bg-white">
      ...
      <div className="flex items-center justify-end border-b border-ink-100 px-3 py-2">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[11px] text-ink-400">{watchlist.length} 只</span>
          <span className="text-[11px] text-ink-300">·</span>
                    ...
                    >
                      <div className="flex-1 min-w-0">
                        <div className="text-xs font-medium truncate">
                          {stock.name}
                          {inWatchlist && <span className="ml-2 text-[10px] text-ink-400">已自选</span>}
                        </div>
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {filteredList.map((w) => {
          const q      = quotes?.find((d) => d.code === w.code);
          const active = selectedCode === w.code;
          return (
            <li
              key={w.code}
        ...
        {filteredList.length === 0 && (
          <li className="px-3 py-6 text-center text-xs text-ink-400">
            {searchQuery ? `无匹配「${searchQuery}」` : '自选股为空，添加一只开始'}
          </li>
        )}
        </ul>
      </div>
